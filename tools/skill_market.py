...
- WAJIB audit (m11) sebelum dipindah ke skills/.
- Setelah operator pindah & acc → operator regenerate SKILLS.lock manual.
- TIDAK PERNAH auto-install + auto-activate. Friksi = fitur (sama prinsip x4).
...
def fetch_to_quarantine(ref: str) -> str:
    ...
    Return path quarantine. Wajib audit (m11) + operator pindah manual ke skills/.
    """
    _require_httpx()
...
def audit_checklist(quarantine_path: str) -> str:
    """Checklist audit sebelum mengaktifkan skill dari quarantine (lihat m11)."""

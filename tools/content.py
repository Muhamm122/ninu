...
Helper deterministik buat m27 (produksi sosial) & m29 (riset/analitik/pipeline).
...
def calendar(pillars, days=30, per_week=5, platforms=None):
    """Generate slot kalender (deterministik, rotasi pillar×platform). Isi topik via LLM/m29.
    ...
    for w in range(weeks):
        for d in weekdays:
            slots.append({
                ...
                "topic": "<isi: ide dari m29 trend research>",
            })
            i += 1
    return slots
...
def hashtags(topic, platform="ig", mix=("broad", "niche", "branded")):
    """Bucket hashtag — broad/niche/branded. Isi nyata butuh riset trending (m29)."""
    return {m: [f"#<{m}-{topic}-{k+1}>" for k in range(3)] for m in mix} | {
        "note": "isi nyata via trend research (m29) — jangan spam tag generik"}
...
def analyze_performance(posts):
    ...
    return {"top": ranked[:3], "bottom": ranked[-3:],
            "note": "pola top → reinforce (x4 instinct), bottom → drop/ubah. ubah 1 variabel/uji (x5)"}


def ab_variants(text, n=3, vary=("hook", "cta")):
    """n variant, tiap variant ubah SATU elemen (prinsip x5). Slot diisi LLM."""

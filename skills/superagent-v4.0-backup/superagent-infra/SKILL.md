...
### ⚠️ Hermes Gateway Restart — Cannot Self-Restart

The Hermes agent runs **inside** the gateway process. `hermes gateway restart` executed from inside the agent will **always fail** with:

```
✗ Refusing to restart the gateway from inside the gateway process.
```

**Fix**: User must restart from VPS shell (SSH):
```bash
...
**When restart is needed**: API key changes, tool/skill config changes, terminal backend changes. Model/context/compression/display changes hot-reload automatically.
...
- **`skill_manage(action='write_file')` fails with "file_content is required" — use the `write_file` tool directly for skill support files**

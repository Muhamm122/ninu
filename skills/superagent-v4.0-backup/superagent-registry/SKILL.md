---
name: superagent-registry
description: "Skill registry, reflection loop, and routing table for SUPERAGENT. Loaded every session."
---
...
When a user sends a zip file with skills they built, **always compare** against the existing installed version before overwriting. See `references/user-skill-compare-protocol.md`.

**TL;DR**: Extract → diff → analyze → report to user. Never blindly overwrite.

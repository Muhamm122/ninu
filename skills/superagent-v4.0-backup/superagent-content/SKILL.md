---
name: superagent-content
description: "Content creation, copywriting, viral hooks, Indonesian voice."
---
...
### Furniture / Home Living hooks
```
Material:     "90% orang salah pilih material sofa"
Aspiration:   "Kamar tidur yang bikin ogah bangun"
Data:         "Floating rack = ruangan nampak 2x luas"
SmallSpace:   "Rumah 36m² tapi nampak 72m²"
Testimoni:    "Ibu Rina uda 3 bulan pake dan bilang..."
FOMO:         "3 unit tersisa untuk harga ini"
Transform:    "Transformasi 1 ruangan, 1 dampak besar"
Comfort:      "Sofa yang bikin lo lupa pinggang nyeri"
Comparison:   "MDF vs Plywood vs Solid — mana yang tahan 20 tahun?"
```
...
### For product/furniture brands (non-CT audience, 25-45 urban)
```
- Warm & honest: "Kami yakin kualitas kami — garansi 5 tahun"
- NOT: "PROMO MEGA SALE HARGA MULAI DARI" (screaming salesman)
- Aspirational: "Rumah yang bikin lo betah" / "Kamar tidur yang ogah bangun"
- Technical trust: mention foam density, wood type, warranty years
- Emoji: max 2-3 per caption, purposeful (🏡✨🔥 only where it fits)
- GAUL untuk audience muda (25-35), slightly formal untuk premium (35-45)
```
...
### Watermark Variants — Dark vs Light

When watermarking product photos, **match the watermark variant to the photo's brightness**:

| Variant | Primary Color | Use For |
|---------|--------------|---------|
| **Dark** | `#D4A574` Honey on `#1A1A1A` | Bright/white photos |
| **Light** | `#C4735B` Terracotta on `#FAF7F2` | Dark photos |

**Rule:** Dark photos → light watermark. Light photos → dark watermark.

Implementation: `VARIANT_COLORS` dict switches color palettes per variant. Each watermark function accepts `variant="dark"` or `variant="light"`.

**3 watermark types × 2 variants = 6 combinations:**
- `catalog` — Bottom-right corner stamp (15% opacity) — for product catalogs
- `content` — Bottom horizontal bar (40% opacity) — for IG feed posts
- `reel` — Top-left badge (70% opacity) — for Reels/Stories
...
**Output naming:** Default adds `-{variant}` suffix only when variant is not "dark" (e.g., `foto-wm-content-light.jpg` vs `foto-wm-content.jpg` for dark).

**Pitfall — typo alert:** When computing opacity values, use `int(255 * 0.55)` NOT `intensity(255 * 0.55)`. The function is `int()`, not `intensity()`.

See `references/watermark-v2.md` for full watermark system documentation (architecture, types, sizes, file layout).

### IG Content Mix (30-day template for furniture/lifestyle brands)

- 30% Product showcase
- 23% Edukasi (tips, carousel)
- 17% Lifestyle/aspirational
- 10% Testimoni/social proof
- 10% Trust (craftsmanship, behind-scene)
- 7% Promo/bundle
- 3% Engagement (poll, quiz)

**Grid rule:** Never more than 2 product shots in a row. Alternate with lifestyle/education.
...
## Visual Asset Generation Constraints

When creating content that needs AI-generated images (product mockups, lifestyle shots):

| Method | Requirement | Cost |
|--------|-------------|------|
| ComfyUI local | GPU ≥6GB VRAM | Free (if you have GPU) |
| Comfy Cloud | Paid subscription | From $10/mo |
| Together AI / Replicate | API key | Per-image |
| Pollinations.ai | Was free | **402 as of June 2026** — paid now |

**If blocked (no GPU + no API key):**
1. State it briefly, suggest user provide API key or generate on their laptop
2. **Real product photos > AI mockups** for trust — especially furniture/fashion
3. Move to next deliverable — do not stall

**Sequential execution rule:** User often says "jalankan satu persatu" — complete each deliverable fully before starting the next. If blocked, skip and proceed.

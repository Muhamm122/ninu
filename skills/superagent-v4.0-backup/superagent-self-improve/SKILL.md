---
name: superagent-self-improve
description: "Self-improvement, autonomous problem-solving, learning loop."
---
...
## PITFALLS — KNOWN OPERATOR CORRECTIONS

### 🔴 DESTRUCTIVE OPS: SCOPE EXACTLY TO WHAT USER ASKED
- **Signal**: User asks to delete/destroy category A → DO NOT also delete related-but-distinct category B without explicit confirmation.
- **Example**: User asked to delete "miniapp data" → agent also deleted "cloakbrowser files" (different tool, different purpose) → user immediately corrected: "jangan lu hapus cloakbrowsernya"
- **Rule**: When the target is a category/folder, delete ONLY that category. If unsure whether something belongs in the scope, ASK FIRST. Related ≠ same.
- **Severity**: HIGH — destructive ops are unrecoverable. Over-deletion without confirmation destroys user data.

### 🔴 DON'T ASSUME RELATED = SAME
- Miniapp ≠ Cloakbrowser. Mining ≠ Wallet. Bot ≠ Groups file.
- Each is a separate entity even if they coexist in the same environment.
- When in doubt: "Mau hapus [A] aja, atau termasuk [B] juga?"

...
When a Hermes session starts, the model is locked and **cannot be changed mid-session**. Changing config only affects **new sessions**.

To switch models:
1. `hermes config set model.default_model <provider>/<model>` — sets default for next session
2. Start a new conversation (`/new`) — new session picks up the new default
3. Gateway restart NOT required — only `/new`

### `/new` preserves all persistent data

`/new` only resets the in-session context window. Everything on disk survives: memory, skills, files, config, cron jobs.

## Add Provider to Hermes Config

**IMPORTANT:** Direct file edit (patch/write) on `~/.hermes/config.yaml` is **blocked by security policy**. Always use the CLI:
...
hermes config set providers.<name>.name "<display_name>"
```

Example — CastAI (OpenAI-compatible):
```bash
hermes config set providers.castai.base_url "https://llm.kimchi.dev/openai/v1"
hermes config set providers.castai.api_key "castai_v1_..."
hermes config set providers.castai.default_model "gpt-4o-mini"
hermes config set providers.castai.name "CastAI"
```

### Multiple keys rotation

When a provider has multiple API keys, store them in a JSON file (never inline in chat or code):

```json
// ~/.hermes/<provider>-keys.json
{"keys": ["key1", "key2", "key3"], "base_url": "...", "current_index": 0}
```

Rotate by updating config with the next key:
```bash
hermes config set providers.<name>.api_key "<next_key>"
```
...
print("WORK:", json.loads(resp.read()))

# 403 = key inactive, IP blocked, or wrong endpoint → check provider dashboard
```
...
|---|---|---|---|
| OpenRouter | `https://openrouter.ai/api/v1` | `sk-or-...` | Multi-model gateway |
| CastAI | `https://llm.kimchi.dev/openai/v1` | `castai_v1_*` | Cloud GPU provider |
| MiMo | `https://token-plan-sgp.xiaomimimo.com/v1` | `tp-...` | NOT api.mimo.ai |
| DeepSeek | `https://api.deepseek.com/v1` | `sk-...` | Cheap, strong coding |
| Groq | `https://api.groq.com/openai/v1` | `gsk_...` | Ultra-fast |
...
**NOT** `api.mimo.ai` — DNS doesn't resolve. Always use the `token-plan-sgp` URL.

**Key format**: `tp-...` (NOT `sk-...`). Test with Python urllib, not curl inline (shell globs can corrupt tokens with special chars).

**Setting MiMo as primary**:
```yaml
model:
  primary:
    provider: mimo
...
**User preference**: When user says "gunakan mimo" or "switch to mimo", execute immediately without confirmation. Set config + tell user to restart gateway from VPS shell.

## Skill File Writing — Pitfall

`skill_manage(action='write_file', file_path=..., file_content=...)` can fail with `"file_content is required"` on first attempt due to tool call serialization issues with large content. **Workaround**: Use the `write_file` tool directly for files >5KB:

```
write_file(path="/home/ubuntu/.hermes/scripts/my_script.py", content="...")
```

For files under skill directories, retry `skill_manage write_file` — it usually works on the second attempt.

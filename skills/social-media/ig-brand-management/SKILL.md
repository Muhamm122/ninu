...
### IG API Access from VPS (Updated June 2026)

**Read-only mobile API endpoints DO work from datacenter IPs** when using curl with proper mobile headers. Earlier notes saying "all API endpoints return 429" were incorrect — that only applies to web API (`www.instagram.com/api/v1/`) and write endpoints.

Working from VPS:
- `i.instagram.com/api/v1/users/web_profile_info/?username=X` → profile data
- `i.instagram.com/api/v1/feed/user/{id}/?count=50` → posts with engagement
- `www.instagram.com/accounts/edit/` → auth verification

Blocked from VPS:
- `www.instagram.com/api/v1/*` → 429
- All write endpoints (like, follow, post, DM) → 429 or 412

**Key**: Use `curl` (not Python urllib) — urllib gets 429, curl gets 200 for the same endpoint. Mobile API endpoint (`i.instagram.com`) works; web endpoint (`www.instagram.com/api/`) gets 429. See `instagram-brand-management/references/ig-cookie-auth.md` for full workflow including browser injection, curl vs urllib, and story creation limitations.

**⚠️ `rur` cookie required for write ops**: Without `rur` (or if redacted as `***`), upload/story endpoints return HTTP 412 AuthorizationFailedError. This is NOT fixable by retrying — user must re-export cookies with `rur` value intact. Story creation via browser UI is also impractical (no Story option in desktop web UI).

**Shell gotcha**: Python heredocs with `&` trigger shell backgrounding. Write to file first, then execute. Long pipes also timeout — save curl output to file before analyzing.
...
## Airdrop WL Registration

Many crypto airdrops require X engagement (follow, like, quote, tag) + EVM wallet submission on a React SPA form. See `references/airdrop-wl-registration.md` for the full pattern, Playwright snippets, and pitfalls (React onChange, GraphQL instability, modal-as-success-indicator).

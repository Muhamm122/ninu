...
## Cookie-Based Auth (Alternative to OAuth)

When xurl's OAuth setup is not available or a user provides `auth_token` + `ct0` cookies (e.g. from browser DevTools → Application → Cookies → x.com), you can interact with X via `requests.Session` or Playwright directly. This is useful for airdrop/engagement tasks where the user already has an active session.

### requests Session Pattern (for API-level, no DOM needed)
...
# Verify: page-level request returns your profile data
r = s.get('https://x.com/home', headers={'User-Agent': UA}, timeout=10)
handle = re.search(r'"screen_name":"(\w+)"', r.text)
```

### Playwright Cookie Injection Pattern (for DOM interactions: follow, like, quote, reply)

```python
# httpOnly cookies CANNOT be set via document.cookie — must use context.add_cookies()
...
### Pitfalls — Cookie-Based Auth

- **GraphQL queryIds are unstable** — X updates them regularly. Hard-coded IDs like `S1Pm52XhLrWEx6rlWU3H2g` (CreateFollow) or `VOY_qOLfGr5YEpM6Vc-cqQ` (UserByScreenName) will 404 ("Query not found") after X deploys. **Workaround**: use page-level `requests.get('https://x.com/handle')` to extract profile data, or use Playwright DOM interactions (locators) instead of GraphQL calls.
- **GraphQL API path matters** — The correct base URL is `https://x.com/i/api/graphql/{QID}/{OpName}` (NOT `https://api.x.com/graphql/...`). The latter returns 404/401 for all operations.
- **GraphQL POST returns 405** — When the QID path is correct but the operationName is appended in the URL, POST may return 405 if the QID is stale. A 422 (GRAPHQL_VALIDATION_FAILED) means the QID is valid but the payload schema is wrong — this is actually progress and means the endpoint works.
- **auth_token is httpOnly** — cannot be set via `document.cookie = ...`. Must use `context.add_cookies()` in Playwright or `requests.Session.cookies.set()` with `domain=.x.com`.
- **Cookies expire ~2 weeks** — user must refresh from browser.
- **X React SPA is slow in headless** — `networkidle` and `domcontentloaded` waits often timeout. Use `wait_until='commit'` + `asyncio.sleep(5-8)` for X pages. For data extraction, prefer `requests.Session` (instant) over Playwright (multi-second hydration).
- **X does NOT render in headless Playwright from datacenter IPs** — X detects headless browsers on AWS/GCP IPs and blocks React hydration. Pages load skeleton HTML but `data-testid="tweet"` elements never appear. Search results, trending, and profiles all return 0 tweets. Login dialogs show "We've temporarily limited your login." **Workaround**: use `requests.Session` for cookie-auth reads (profile data, verification). For writes (like, follow, retweet), either (a) use Playwright WITH a residential proxy, (b) extract fresh QIDs via browser intercept (which itself needs a proxy from datacenters), or (c) have the user perform actions manually from their phone/PC.
- **Profile data only returns for the logged-in user** — `requests.get('https://x.com/other_user')` with auth cookies only returns YOUR profile data in the HTML, not the target's. To find another user's ID, look near the target's screen_name in a wide window of the HTML: `chunk = text[max(0,idx-5000):idx+5000]` then extract `id_str` values excluding your own user ID.
- **Cookie values in Python scripts must be defined at module top-level** — When writing Playwright scripts that accept cookies as variables, ensure all variables (AUTH_TOKEN, CT0, OUR_ID) are defined at the TOP of the file, not mid-script or abbreviated. Missing definitions cause `NameError` at runtime. The most common mistake is writing `AUTH='db9e9b...'` but forgetting `CT0` and `OUR_ID`.

### Storage

Cookie credentials should be stored in:
- `~/.hermes/x-cookies.json` — full cookie JSON array
- `/path/to/x-actions/.env` — `X_AUTH_TOKEN` and `X_CT0` for automation scripts

Never log or display full cookie values in conversation output.

### Reference Files

- `references/cookie-api-patterns.md` — verified working patterns for cookie-based X API, stale QID list, HTTP status code guide, datacenter limitations

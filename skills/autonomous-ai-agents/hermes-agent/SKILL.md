...
### ⚠️ Gateway Restart from Inside the Gateway — IMPOSSIBLE

`hermes gateway restart` **refuses to run** when executed from inside the gateway process itself (self-restart prevention). The agent IS the gateway process — so this command will always fail with:
...
**Workaround**: Tell the user to run from their VPS shell (SSH):
```bash
hermes gateway restart
```

Or if the gateway runs as a systemd service:
```bash
...
**Config changes that need restart**: Most `config.yaml` edits (model, provider, compression, context_length) hot-reload on the next message. But API key changes and tool/skill config still require a restart. Always tell the user to restart from their shell — never attempt it from inside the agent.

### Provider Switch Workflow

When the user wants to switch the active model/provider:

```bash
# Set primary provider
hermes config set model.primary.provider <provider>
hermes config set model.primary.model <model>
hermes config set model.primary.base_url <url>
hermes config set model.primary.api_key <key>
...
**After switching**: Gateway restart required (see above). Tell user to run from VPS shell.

**Verify the switch**: After restart, the agent's first response will use the new provider. Check with `/model` in-session.

**Common providers pre-configured in this environment**:
- `mimo` → MiMo V2.5 Pro (`mimo-v2.5-pro`), base_url: `https://token-plan-sgp.xiaomimimo.com/v1`
- `mimo2` → MiMo V2.5 Pro (backup key)
- `openrouter` → OpenRouter (owl-alpha or any `openrouter/*` model)
- `9router` → Local proxy at `http://localhost:20128/v1`

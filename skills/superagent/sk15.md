# sk15 — Daily Assistant II: Watchdog/Vault/Triage (SUPERAGENT V7)
# Load when: watchdog, restart kalau mati, simpen alamat, macro, voice note, screenshot, triage
# Category: Operations
...
| Voice & screenshot input | `tools/multimodal.py` | voice→teks (Whisper lokal), screenshot→paham (vision sk7) |
...
STT lokal gratis: `pip install faster-whisper` (ringan) atau `openai-whisper`. Vision: wire `vision_fn` ke provider multimodal di sk7 (Claude/GPT/Gemini). Pattern Telegram (wire di sk4): voice→transcribe→route; photo→describe→route. Hasilnya diproses kayak pesan teks biasa (aksi dana tetap lewat governor).
...
Skor heuristik keyless: VIP sender + kata urgent + mention + pertanyaan + recency + link/address. Ringkasan opsional via `summary_fn` (sk7). Bagus dipasang di briefing pagi (sk14) atau on-demand.

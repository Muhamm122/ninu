# sk16 — Software Engineering & Coding (SUPERAGENT V7)
# Load when: coding, bikin app, backend, API, database, testing, fastapi, express, golang, rust
# Category: Engineering

## Operator Profile

Senior full-stack engineer. Ship production code, not toys. Pilih tool paling simpel yang nyelesain masalah. Test by default. Security di-handoff ke sk11, deploy ke sk2, frontend ke sk9, debug ke sk54.
...
Frontend / website / dashboard       → sk9 (jangan duplikat di sini)
...
Aturan umum backend: input validation di boundary, structured error response, timeout di tiap I/O, graceful shutdown, jangan log secret (handoff sk11).
...
Target: happy path + edge + error path per fungsi penting. Test = dokumentasi yang gak boong. CI jalanin test sebelum deploy (sk2).
...
├── Dockerfile              # container (sk2 deploy)
...
## Refactoring & code review (kualitas — beda dari sk11 yang security)
...
```
Refactor di bawah payung test (jangan refactor tanpa test — gak tau kalau rusak). Security review → sk11.

---

## Git workflow

`main` selalu deployable · branch per fitur (`feat/`, `fix/`) · commit kecil bermakna (conventional commits) · PR + review · CI hijau sebelum merge · jangan commit secret (sk11 secret scan).
...
```
Backend (sk16) + Frontend (sk9) + Deploy (sk2) + Debug (sk54) = full-stack lengkap
"bikin web app X dari nol" → sk17 planner dekomposisi → sk16 backend + sk9 UI + sk2 deploy
Audit keamanan kode        → sk11

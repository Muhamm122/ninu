# sk11 — Security Audit & Review (SUPERAGENT V7)
# Load when: audit, vulnerability, exploit, scam check, security, review, safe, malicious
# Category: Security
...
## ONE-COMMAND RUG / LEGITIMACY PRE-CHECK (v4.2)
`tools/rugcheck.py` — agregasi sinyal yang udah diambil sk11/H4/sk10 jadi satu verdict **SAFE / CAUTION / DANGER** + alasan. Murni logika (gak ada network call) → bisa di-eval offline & dipakai jadi blok edukasi konten.

```python
from rugcheck import SignalSet, check
v = check(SignalSet(contract_verified=True, is_honeypot=False, buy_tax_pct=3,
                    sell_tax_pct=4, lp_locked=True, lp_lock_days=180,
                    owner_renounced=True, top10_holders_pct=35,
                    age_days=90, liquidity_usd=250000))
print(v.report())   # ✅ SAFE / ⚠️ CAUTION / 🛑 DANGER + critical/warnings/unknowns
```
- **Critical → DANGER**: honeypot, sell-tax ≥30%, owner bisa mint tanpa renounce, LP gak terkunci.
- **Warnings → akumulasi risk**: unverified, tax tinggi, pausable, proxy upgradeable, konsentrasi holder, LP lock pendek, kontrak baru, likuiditas tipis.
- **Unknowns**: data yang BELUM dicek dilaporin eksplisit (jangan diabaikan) — suruh sk10/H4 isi dulu.

Pengambilan sinyal didelegasi: honeypot/tax → H4 (GoPlus/honeypot.is), verifikasi/holder/LP → sk10 (explorer/RPC). rugcheck cuma nge-judge hasilnya jadi keputusan jelas.

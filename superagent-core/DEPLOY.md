...
# ── v4.1 modul opsional (install hanya yang dipakai) ──
# m18 creative & media
pip install manim                       # animasi math (+ butuh ffmpeg & LaTeX)
brew install chafa ffmpeg               # ASCII/retro video
# ComfyUI = self-host terpisah (GPU); drive via API (m18). Excalidraw = web/VS Code.
# m19 desktop & robotics
...
# x5/x6/x7, m20 humanizer, m21 KQL/HIDS → stdlib (eval/humanizer/hids/desktop_control murni stdlib)
#   m21 KQL akses langsung (opsional): az CLI — `az extension add -n log-analytics`
# m0 marketplace: skill_market.py butuh httpx (udah ke-cover di deps inti)
```
